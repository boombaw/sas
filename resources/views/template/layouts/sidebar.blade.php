        ...
        <div class="nano-content">
            <ul id="mainnav-menu" class="list-group">
                @foreach ($menuHeader as $header)
                    ...
                    @if ($hasChild == 0 )
                        <!--Menu list item-->
                        <li> <a href="index.html"> <i class="fa fa-home"></i> <span class="menu-title"> {{ ucfirst(substr($header->menu_name, 10, strlen($header->menu_name)))}} </span> </a> </li>
                    @else
                        <!--Menu list item-->
                        <li>
                            <a href="#">
                             ...
                             <ul class="collapse">
                                @foreach ($menuChild as $child)
                                    @if ($child->menu_parent == $header->menu_id)
                                        <li><a href="layout-blank.html"><i class="fa fa-circle-thin"></i> {{ ucfirst(substr($child->menu_name, 10, strlen($child->menu_name)))}} </a></li>
                                    @endif
                                @endforeach
                            </ul>
                        </li>
